#Logic:Base case if Tree is None then return -1
    # ->find the largest of left ,and largest of right ,and compare both left,right large value 
    #and then compare that large value with the root value and return the large value 
...
#Node with large data
def largestData(root):
    if root == None:
        return -1 #ideally need to return -infy but here we are returning -1
    leftlargest = largestData(root.left)
    rightlargest = largestData(root.right)
    #simply finding the max out of all left,right ,and root value
...
print("the largest data is ",largestData(root))
